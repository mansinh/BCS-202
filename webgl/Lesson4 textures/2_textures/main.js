...
var shader = {
    ...
    propertyLocationTextureA:null,
    propertyLocationTextureB:null,
    vertexSrc :
        `#version 300 es
    ...
    fragmentSrc : 
        ...
        void main(){
            finalColor = linkedColor*texture(textureB,linkedTexCoord);
        }`
};
...
var textureBufferB = GL.createTexture();
textureBufferB.image = new Image();
textureBufferB.image.src = "./images/Beagle.jpg";
textureBufferB.image.onload = function(){
    GL.bindTexture(GL.TEXTURE_2D,textureBufferB);
    GL.texImage2D(GL.TEXTURE_2D,0,GL.RGB,GL.RGB,GL.UNSIGNED_BYTE,textureBufferB.image);
    GL.texParameteri(GL.TEXTURE_2D, GL.TEXTURE_MAG_FILTER,GL.NEAREST);
    GL.texParameteri(GL.TEXTURE_2D, GL.TEXTURE_MIN_FILTER,GL.NEAREST);

    console.log(textureBufferB.image.complete);
    start();
}

function start(){
    if(!textureBufferA.image.complete || !textureBufferB.image.complete) return;
...
function getPropertyLocations(){
    ...
    shader.propertyLocationTextureA = GL.getUniformLocation(shader.program,"textureA");
    shader.propertyLocationTextureB = GL.getUniformLocation(shader.program,"textureB");
...
function setShaderProperties(){
    ...
    GL.bindTexture(GL.TEXTURE_2D,textureBufferA);
    GL.uniform1i(shader.propertyLocationTextureA,0);

    GL.activeTexture(GL.TEXTURE1);
    GL.bindTexture(GL.TEXTURE_2D,textureBufferB);
    GL.uniform1i(shader.propertyLocationTextureB,0);
